var trueAnswer = 0;
    ...
    function init(rawCSV) {
	  ...
	  //BUG sur les stats
      document.querySelector('#raz').addEventListener('click', () => { localStorage.removeItem('questionsData'); location.reload(); document.querySelector('#menuToggle input').checked = false;});
	  ...
	  document.querySelector('#choosePackage').addEventListener('click', () => {
		  document.querySelector('fieldset#packageSelector').innerHTML='<legend>Quels paquets reviser ?</legend>';
		  populatePackageSelector(questionsArray);
		  document.querySelector('#packageSelectorDiv').classList.remove('hidden');
		  document.querySelector('#menuToggle input').checked = false; 
		  });
      document.querySelector('#validatePackage').addEventListener('click', () => { start(questionsArray) });
	  document.querySelectorAll('#packageSelectorDiv input[type=checkbox]').forEach((e)=>{e.addEventListener('click', ()=> {
		


        var checkboxes = document.querySelectorAll(
          '#packageSelectorDiv input[type="checkbox"]'
        );
        var checkedOne = Array.prototype.slice
          .call(checkboxes)
          .some((x) => x.checked);
        if (checkedOne) {
          document.querySelector("button#validatePackage").disabled = false;
        } else {
          document.querySelector("button#validatePackage").disabled = true;
        }
    ...
    function populatePackageSelector(qArray) {
      ...
      packageList.forEach((el) => {
        ...
        label.innerText = el;
        label.innerHTML += "<span id='packageStat'> ( <span title='questions pour plus tard' style='color:green'>" + kq +
          "</span> − <span title='questions pour maintenant'  style='color:blue'>" +
          dq +
          "</span> − <span title='questions non vues' style='color:#555'>" +
          ns + " )</span>";
    ...
    function getStat(questionsArray) {
      ...
      document.querySelector("#stat").innerHTML =
        "<span title='questions pour plus tard' style='color:green'>" +
        kq +
        "</span> − <span title='questions pour maintenant'  style='color:blue'>" +
        dq +
